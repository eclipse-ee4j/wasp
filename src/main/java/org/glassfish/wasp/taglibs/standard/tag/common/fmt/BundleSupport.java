...
public abstract class BundleSupport extends BodyTagSupport {
     ...
     * base name.
     *
     * @param pc Page in which the resource bundle with the given base name is requested
    ...
    private static ResourceBundle findMatch(String basename, Locale pref) {
        ...
        try {
            ...
            if (pref.equals(avail)) {
                ...
                /*
                 * We have to make sure that the match we got is for the specified locale. The way ResourceBundle.getBundle() works, if
                 * a match is not found with (1) the specified locale, it tries to match with (2) the current default locale as returned
                 * by Locale.getDefault() or (3) the root resource bundle (basename). We must ignore any match that could have worked
                 * with (2) or (3). So if an exact match is not found, we make the following extra tests: - avail locale must be equal
                 * to preferred locale - avail country must be empty or equal to preferred country (the equality match might have failed
                 * on the variant)
                 */
                if (pref.getLanguage().equals(avail.getLanguage())
                        && ("".equals(avail.getCountry()) || pref.getCountry().equals(avail.getCountry()))) {
                    /*
                     * Language match. By making sure the available locale does not have a country and matches the preferred locale's
                     * language, we rule out "matches" based on the container's default locale. For example, if the preferred locale is
                     * "en-US", the container's default locale is "en-UK", and there is a resource bundle (with the requested base name)
                     * available for "en-UK", ResourceBundle.getBundle() will return it, but even though its language matches that of the
                     * preferred locale, we must ignore it, because matches based on the container's default locale are not portable across
                     * different containers with different default locales.
                     */
                    match = bundle;
                }
            }
