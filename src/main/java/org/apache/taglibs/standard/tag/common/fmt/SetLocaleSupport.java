...
public abstract class SetLocaleSupport extends TagSupport {
    ...
    /**
     * Sets up the available formatting locales that will be used by getFormattingLocale(PageContext).
     */
    static {
    ...
    static void setResponseLocale(PageContext pc, Locale locale) {
        ...
        if (pc.getSession() != null) {
            try {
                pc.setAttribute(RequestEncodingSupport.REQUEST_CHAR_SET, response.getCharacterEncoding(), PageContext.SESSION_SCOPE);
    ...
    static Locale getFormattingLocale(PageContext pc, Tag fromTag, boolean isDate, boolean format) {
        ...
        if (parent != null) {
            /*
             * use locale from localization context established by parent <fmt:bundle> action, unless that locale is null
             */
     ...
     * ("exact match"). If no exact match exists, the best match is given to an available locale that meets the following
     * criteria (in order of priority): - available locale's variant is empty and exact match for both language and country
