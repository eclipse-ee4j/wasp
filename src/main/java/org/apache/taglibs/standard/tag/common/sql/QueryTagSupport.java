            ...
            if (obj != null) {
                if (obj instanceof Integer) {
                    ...
                    try {
                        ...
                        throw new JspException(Resources.getMessage("SQL_MAXROWS_PARSE_ERROR", obj), nfe);
                    }
                } else {
    ...
    private void setParameters(PreparedStatement ps, List<Object> parameters) throws SQLException {
        if (parameters != null) {
            for (int i = 0; i < parameters.size(); i++) {
                /*
                 * The first parameter has index 1. If a null is passed to setObject the parameter will be set to JDBC null so an
                 * explicit call to ps.setNull is not required.
                 */
