...
public abstract class TransformSupport extends BodyTagSupport {
    ...
    /**
     * Wraps systemId with a "jstl:" prefix to prevent the parser from thinking that the URI is truly relative and resolving
    ...
    /**
     * Retrieves a Source from the given Object, whether it be a String, Reader, Node, or other supported types (even a
     * Source already). If 'url' is true, then we must be passed a String and will interpret it as a URL. A null input
    ...
    /**
     * A Writer based on a wrapped Writer but ignoring requests to close() and flush() it. (Someone must have wrapped the
