...
/**
 * <p>
 * Support for tag handlers for &lt;import&gt;, the general-purpose text-importing mechanism for JSTL 1.0. The
 * rtexprvalue and expression- evaluating libraries each have handlers that extend this class.
 * </p>
 *
 * @author Shawn Bayern
 */
...
public abstract class ImportSupport extends BodyTagSupport implements TryCatchFinally, ParamParent {
     ...
     * characters plus ("+"), period ("."), and hyphen ("-") are allowed. For resiliency, programs interpreting URLs should
    ...
    /*
     * Overall strategy: we have two entry points, acquireString() and acquireReader(). The latter passes data through
     * unbuffered if possible (but note that it is not always possible -- specifically for cases where we must use the
     * RequestDispatcher. The remaining methods handle the common.core logic of loading either a URL or a local resource.
     *
     * We consider the 'natural' form of absolute URLs to be Readers and relative URLs to be Strings. Thus, to avoid doing
     * extra work, acquireString() and acquireReader() delegate to one another as appropriate. (Perhaps I could have spelled
     * things out more clearly, but I thought this implementation was instructive, not to mention somewhat cute...)
     */
    ...
    private class ImportResponseWrapper extends HttpServletResponseWrapper {
         ...
         *
         * In other words, the target's asserted encoding is used to convert from a Writer to an OutputStream, which is
         * typically the medium through with the target will communicate its ultimate response. Since we short-circuit that
         * mechanism and read the target's characters directly if they're offered as such, we simply ignore the target's
         * encoding assertion.
         */
        ...
        /**
         * Retrieves the buffered output, using the containing tag's 'charEncoding' attribute, or the tag's default encoding,
         * <b>if necessary</b>.
         */
