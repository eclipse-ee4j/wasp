...
class Parser implements TagConstants {
    ...
    public static Node.Nodes parse(ParserController pc, String path, JspReader reader, Node parent, boolean isTagFile, boolean directivesOnly, URL jarFileUrl,
            String pageEnc, String jspConfigPageEnc, boolean isDefaultPageEncoding, boolean hasBom) throws WaspException {
     ...
     */
    Attributes parseAttributes() throws WaspException {
     ...
     */
    public static Attributes parseAttributes(ParserController pc, JspReader reader) throws WaspException {
     ...
     */
    private boolean parseAttribute(AttributesImpl attrs) throws WaspException {
     ...
     */
    private String parseName() throws WaspException {
     ...
     */
    private String parseAttributeValue(String watch) throws WaspException {
     ...
     */
    private void processIncludeDirective(String file, Node parent) throws WaspException {
        if (file == null) {
            return;
        }
     ...
     */
    private void parsePageDirective(Node parent) throws WaspException {
     ...
     */
    private void parseIncludeDirective(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
     ...
     */
    private void addInclude(Node parent, List files) throws WaspException {
        if (files != null) {
     ...
     */
    private void parseTaglibDirective(Node parent) throws WaspException {
        ...
        if (prefix != null) {
            ...
            if (uri != null) {
                 ...
                 * (WaspException je) { err.throwException(reader.mark(), je); } pageInfo.addTaglib(uri, taglib); }
                 */
                // START GlassFish 750
     ...
     */
    private void parseDirective(Node parent) throws WaspException {
        reader.skipSpaces();
     ...
     */
    private void parseXMLDirective(Node parent) throws WaspException {
        reader.skipSpaces();
     ...
     */
    private void parseTagDirective(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
     ...
     */
    private void parseAttributeDirective(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
     ...
     */
    private void parseVariableDirective(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
     ...
     */
    private void parseComment(Node parent) throws WaspException {
        start = reader.mark();
     ...
     */
    private void parseDeclaration(Node parent) throws WaspException {
        start = reader.mark();
        Mark stop = reader.skipUntil("%>");
        if (stop == null) {
            err.jspError(start, "jsp.error.unterminated", "&lt;%!");
     ...
     */
    private void parseXMLDeclaration(Node parent) throws WaspException {
        reader.skipSpaces();
        if (!reader.matches("/>")) {
            if (!reader.matches(">")) {
     ...
     */
    private void parseExpression(Node parent) throws WaspException {
        start = reader.mark();
        Mark stop = reader.skipUntil("%>");
        if (stop == null) {
            err.jspError(start, "jsp.error.unterminated", "&lt;%=");
     ...
     */
    private void parseXMLExpression(Node parent) throws WaspException {
        reader.skipSpaces();
        if (!reader.matches("/>")) {
            if (!reader.matches(">")) {
     ...
     */
    private void parseELExpression(Node parent, String typeEL) throws WaspException {
        start = reader.mark();
     ...
     */
    private void parseScriptlet(Node parent) throws WaspException {
        start = reader.mark();
        Mark stop = reader.skipUntil("%>");
        if (stop == null) {
            err.jspError(start, "jsp.error.unterminated", "&lt;%");
     ...
     */
    private void parseXMLScriptlet(Node parent) throws WaspException {
        reader.skipSpaces();
        if (!reader.matches("/>")) {
            if (!reader.matches(">")) {
     ...
     */
    private void parseParam(Node parent) throws WaspException {
     ...
     */
    private void parseInclude(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
     ...
     */
    private void parseForward(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
    ...
    private void parseInvoke(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
    ...
    private void parseDoBody(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
    ...
    private void parseElement(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
     ...
     */
    private void parseGetProperty(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
     ...
     */
    private void parseSetProperty(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
     ...
     * EmptyBody ::= '/>' | ( '>' ETag ) | ( '>' S? '<jsp:attribute' NamedAttributes ETag )
     */
    private void parseEmptyBody(Node parent, String tag) throws WaspException {
        if (reader.matches("/>")) {
            // Done
        } else if (reader.matches(">")) {
     ...
     */
    private void parseUseBean(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
     ...
     *
     */
    private void parseOptionalBody(Node parent, String tag, String bodyType) throws WaspException {
        if (reader.matches("/>")) {
            // EmptyBody
            return;
        }
     ...
     */
    private boolean parseJspAttributeAndBody(Node parent, String tag, String bodyType) throws WaspException {
     ...
     * '</jsp:params>'
     */
    private void parseJspParams(Node parent) throws WaspException {
     ...
     */
    private void parseFallBack(Node parent) throws WaspException {
     ...
     *
     */
    private void parsePlugin(Node parent) throws WaspException {
        Attributes attrs = parseAttributes();
        reader.skipSpaces();
     ...
     */
    private void parsePluginTags(Node parent) throws WaspException {
        reader.skipSpaces();
     ...
     */
    private void parseStandardAction(Node parent) throws WaspException {
        Mark start = reader.mark();
     ...
     */
    private boolean parseCustomTag(Node parent) throws WaspException {
     ...
     */
    private void parseTemplateText(Node parent) throws WaspException {
     ...
     */
    private void parseXMLTemplateText(Node parent) throws WaspException {
        reader.skipSpaces();
        if (!reader.matches("/>")) {
            if (!reader.matches(">")) {
     ...
     */
    private void parseElements(Node parent) throws WaspException {
     ...
     */
    private void parseElementsScriptless(Node parent) throws WaspException {
     ...
     */
    private void parseElementsTemplateText(Node parent) throws WaspException {
        start = reader.mark();
        if (reader.matches("<%--")) {
            parseComment(parent);
        } else if (reader.matches("<%@")) {
     ...
     */
    private void checkUnbalancedEndTag() throws WaspException {
     ...
     * TagDependentBody :=
     */
    private void parseTagDependentBody(Node parent, String tag) throws WaspException {
     ...
     */
    private void parseJspBody(Node parent, String bodyType) throws WaspException {
        Mark start = reader.mark();
     ...
     */
    private void parseBody(Node parent, String tag, String bodyType) throws WaspException {
        if (bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_TAG_DEPENDENT)) {
            ...
            if (!reader.matchesETag(tag)) {
                err.jspError(start, "wasp.error.emptybodycontent.nonempty", tag);
            }
     ...
     */
    private void parseNamedAttributes(Node parent) throws WaspException {
        do {
            Mark start = reader.mark();
            Attributes attrs = parseAttributes();
    ...
    private void parseTagFileDirectives(Node parent) throws WaspException {
